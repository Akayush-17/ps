...
from .models import LikePost, Profile, Post, FollowersCount, Comment
...
def logout(request):
    ...
    auth.logout(request)
    return redirect('https://de-verese.github.io/Start/')
...
def bookservice(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            html = render_to_string('email_template.html', {
                'name': name,
                'username': username,
                'email': email,
                'content': content
            ...
            send_mail('The contact form subject', 'This is the message',
                      'ayushkumar_21387@aitpune.edu.in', ['aaayushsinghh@gmail.com'], html_message=html)
...
def comment(request):
    ...
    if request.method == 'POST':
        ...
        new_comment = Comment.objects.create(user=user, comment1=comment1)
        new_comment.save()
